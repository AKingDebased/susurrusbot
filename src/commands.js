...
const commands = {
    ...
    hello: ({ client, channel, userstate }) => {
        ...
        switch (userstate['display-name'].toLowerCase()) {
            ...
            case 'dondeenie':
                ...
                return;
            case 'joydrop':
                client.say(channel, `So good to have you here, joydrop. Esteemed member of the Council of Three. Sampled any tasty IPAs recently? Or are you more of a stout guy? Me, I've always been partial to lagers.`);
                return;
            case 'lockdown1919':
                client.say(channel, `Ah, if it isn't lockdown1919, Lord of Bits. Oh, I'm sorry, I meant Locky D. Am I saying that right? Well, in any case, glad you could make it!`);
                return;
            case 'shaneshane':
    ...
    commands: ({ client, channel, userstate }) => {
        client.say(channel, `/me flips through her notebook. "At your subscription level, ${userstate['display-name']}, you have access to !hello, !holidays, !so, and !susu."`);
    },
    // mod commands
    susutimeout: ({ client, channel, userstate, userCommandArgs }) => {

        if (!userstate.mod && userstate['display-name'] !== process.env.BROADCASTER_NAME) {
            client.say(channel, `/me wags her pen back and forth, admonishingly. "Sorry, ${userstate['display-name']}, my love, but I don't take those kinds of orders from just anyone."`);
            return;
        }
        ...
        client.say(channel, `/me twirls her pen with a flourish, before pressing it firmly over your mouth. "Now, now, ${timeoutUser}, I'm going to have to ask you to not do that again. Why don't you take a moment?"`);
    },
}
