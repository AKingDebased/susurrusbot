...
const commands = {
    ...
    //     client.say(channel, `/timeout faithlessfew`);
    //     client.say(channel, `Now, now, @faithlessfew, I'm going to have to ask you to not do that again.`);
    // },
...
const parseCommands = ({ client, channel, userstate, message }) => {
    ...
    if (userCommandSymbol && Object.keys(commands).includes(userCommandName)) {
        if (userCommandArgs.length) {
            // console.log('command had arguments!');
        } else {
            // console.log('command had no arguments!');
        }
